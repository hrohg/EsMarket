...
namespace ES.Common.Helpers
{
    public class Ean13
    {
        ...
        public Ean13(string barcode)
        {
            if(string.IsNullOrWhiteSpace(barcode) || barcode.Length!=13) return;
            CountryCode = barcode.Substring(0, 2);
            ManufacturerCode = barcode.Substring(2, 5);
            ProductCode = barcode.Substring(7, 5);
            ChecksumDigit = barcode.Substring(12, 1);
        }
