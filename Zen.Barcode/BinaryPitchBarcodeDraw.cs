...
namespace Zen.Barcode
{
        ...
        where TChecksum : Checksum
    {
        ...
        protected override void RenderBar(int glyphIndex, BarGlyph glyph, DrawingContext dc,
            Rect bounds, ref double barOffset, double barMinHeight,
            double barMinWidth, double barMaxWidth)
        {
            ...
            if (Math.Abs(barMinWidth - barMaxWidth) < EPSILON)
            {
                throw new InvalidOperationException("Only variable-pitch drawing supported.");
            }
            if (WidthBitCount == 0)
            ...
            for (int bitIndex = encodingBitCount - 1; bitIndex >= 0; --bitIndex)
            {
                ...
                {
                    dc.DrawRectangle(Brushes.Black, null, new Rect(barOffset, bounds.Top,
