...
namespace Zen.Barcode
{
		...
		: BarcodeDrawBase<Code128GlyphFactory, Code128Checksum>
	{
		...
		{
            double maxHeight = desiredBarcodeDimensions.Height * printResolution.Height / 100;
            double narrowBarWidth = (printResolution.Width * desiredBarcodeDimensions.Width) /
				(500 * (24 + (barcodeCharLength * 11)));
		...
		/// <returns></returns>
        protected override double GetBarcodeLength(Glyph[] barcode,
            double interGlyphSpace, double barMinWidth, double barMaxWidth)
		{
